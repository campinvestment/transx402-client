...
- Type-check the runtime test that passes `configProxyPath` with `settlement: "direct"` so CI `tsc --noEmit` succeeds
...
- `configProxyPath` option for server settlement — same-origin merchant proxy for `GET /config`
- `DEFAULT_CONFIG_PROXY_PATH` constant (`/api/transx402`)

## [0.2.0] — 2026-07-27

### Added

- `settlement?: "server" | "direct"` on client options
  - `fetch()` defaults to **`server`** (canonical): sign → `PAYMENT-SIGNATURE` retry; merchant backend calls `/facilitate` via `@transx402/server`
  - `pay()` and `createPaywall()` always use **`direct`** (client calls `/facilitate`)

### Changed

- Breaking for apps that relied on `fetch()` settling with the facilitator before retry.
  Set `settlement: "direct"` to keep the previous behavior, or pair with `@transx402/server` on the merchant API.
...
[0.2.0]: https://github.com/campinvestment/transx402-client/compare/v0.1.4...v0.2.0
